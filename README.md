...
### 中国建设银行信用卡（`ccb_credit_card`）

保存中国建设银行发送至邮箱的电子账单 EML 文件。
...
### 浦发银行借记卡（`spdb_debit_card`）

在手机银行客户端申请交易流水打印（个人客户交易流水专用回单），下载邮件附件，得到 PDF 文件。

如 PDF 带有密码，把密码记录到 `config.py` 的 `pdf_passwords` 中。
